...
int main() {
    ...
    struct timespec start_init, end_init, start_add, end_add, start_verify, end_verify;
    double time_init, time_add, time_verify;
    
    // Obter o dispositivo atual e o host
    int device = -1;
    cudaGetDevice(&device);
    int cpu = cudaCpuDeviceId;
    ...
    // Experimento E: Prefetch todos os vetores para o dispositivo
    printf("Realizando prefetch de todos os vetores 'a', 'b' e 'c' para o dispositivo...\n");
    ...
    // Prefetch o vetor de resultado de volta para a CPU antes da verificação
    printf("Realizando prefetch do vetor 'c' de volta para a CPU para verificação...\n");
    clock_gettime(CLOCK_MONOTONIC, &start_verify);
    
    cudaMemPrefetchAsync(c, size, cpu, NULL);
    
    // Verificar resultados
    printf("Verificando resultados...\n");
    checkElementsAre(7.0f, c, N);
    
    clock_gettime(CLOCK_MONOTONIC, &end_verify);
    time_verify = getElapsedTime(start_verify, end_verify);
    printf("Tempo de verificação: %.2f ms\n", time_verify);
    ...
    printf("Tempo total: %.2f ms\n", time_init + time_add + time_verify);
